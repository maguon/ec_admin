...
export const updatePaymentStatus=(id,params)=>async (dispatch, getState) => {
    try {
        const url = apiHost + '/api/user/' + localUtil.getSessionItem(sysConst.LOGIN_USER_ID) + '/payment/'+id+'/status?status=' + params;
        dispatch({type: AppActionType.showLoadProgress, payload: true});
